    ...
    useEffect(()=>{
        const fetchUserHistories = async()=>{
            if(authState && authState.isAuthenticated){
                ...
                if(!response.ok){
                    throw new Error("something went wrong in fetchUserHistories");
                }
                const responseJson = await response.json();
                console.log(responseJson._embedded.histories);
        ...
        fetchUserHistories().catch((error)=>{
            setHttpError(error);
            setIsLoadingHistory(false);
        })
                            ...
                            <div className={'row g-0'}>
                                <div className={'col-md-2'}>
                                    <div className={"d-none d-lg-block"}>
                                        {history.img?
                                            <img src={history.img} width={'123'} height={'196'}
                                                 alt={"Book"}/>
                                            :
                                            <img src={
                                                require("./../../../Images/BooksImages/book-luv2code-1000.png")} width={'123'} height={'196'}
                                                 alt={"Default"}/>
                                        }
